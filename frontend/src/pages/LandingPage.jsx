...
export default function LandingPage({ onAuthSuccess, theme }) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col justify-between font-sans relative overflow-hidden select-none font-jakarta">
      ...
      <div className="w-full max-w-6xl mx-auto px-6 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <button 
            ...
            className="text-sm font-bold text-slate-500 hover:text-slate-800 transition-colors cursor-pointer bg-transparent border-0 outline-none"
          >
            Sign In
          </button>
          ...
          <button 
            ...
            className="bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold px-4 py-2 rounded-xl transition-all cursor-pointer shadow-xs border-t border-white/20 border-x border-white/10"
          >
            Get Started
          </button>
        </div>
      ...
      <main className="flex-1 w-full max-w-6xl mx-auto px-6 pb-12 md:pb-20 pt-8 flex flex-col lg:flex-row items-center gap-16 relative z-10 justify-center">
        ...
        <div className="flex-1 flex flex-col text-left gap-6 lg:max-w-xl">
          ...
          {/* Release Badge */}
          <div className="inline-flex items-center gap-2 bg-blue-50 border border-blue-100/50 text-blue-700 rounded-full py-1.5 px-4 w-fit shadow-2xs">
          ...
          <form 
            ...
            className="w-full bg-white border border-slate-200 rounded-2xl p-2 flex items-center shadow-[0_20px_48px_-12px_rgba(15,23,42,0.12),0_4px_16px_rgba(15,23,42,0.02)] focus-within:border-blue-500/80 focus-within:shadow-[0_20px_48px_-12px_rgba(37,99,235,0.14)] transition-all"
          >
            <input
              type="text"
              value={promptText}
              ...
              className="flex-1 bg-transparent border-0 px-3 text-slate-800 placeholder-slate-400 outline-none text-sm font-medium"
            />
            <button
              type="submit"
              className="bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs py-2.5 px-5 rounded-xl transition-all cursor-pointer flex items-center gap-1.5 shadow-sm border-t border-white/20 border-x border-white/10 active:scale-95"
            >
          ...
          {/* Core highlights */}
          <div className="flex items-center gap-6 mt-2 text-slate-400 text-xs font-bold uppercase tracking-wider">
        ...
        <div className="flex-1 w-full max-w-lg lg:max-w-none relative flex justify-center">
          ...
          <div className="absolute inset-0 bg-gradient-to-r from-blue-500/5 to-cyan-500/5 blur-3xl rounded-full scale-110 pointer-events-none" />

          {/* The visual deck screen */}
          <div className="w-full max-w-md aspect-video bg-white rounded-2xl shadow-[0_32px_80px_rgba(15,23,42,0.14)] border border-slate-200/80 p-5 flex flex-col justify-between relative overflow-hidden group select-none">
            
            {/* Interactive master slide design elements */}
            {mockSlides[activeMockIndex].accent === "fluent" && (
              <>
                <div className="absolute -top-12 -right-12 w-48 h-48 rounded-full bg-gradient-to-br from-cyan-400/20 to-blue-600/20 blur-xl pointer-events-none z-0" />
                <div className="absolute -bottom-16 -left-16 w-60 h-60 rounded-full bg-gradient-to-tr from-purple-500/10 to-indigo-500/10 blur-xl pointer-events-none z-0" />
            ...
            {mockSlides[activeMockIndex].accent === "card" && (
              <div className="absolute inset-4 rounded-xl border border-slate-900/[0.04] bg-slate-50/60 z-0 pointer-events-none shadow-2xs" />
            )}

            {/* Slide Header preview */}
            <div className="flex items-center justify-between border-b border-slate-100/50 pb-3 relative z-10">
      ...
      <footer className="w-full py-6 border-t border-slate-200/60 bg-white/60 relative z-10">
        <div className="max-w-6xl mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-4 text-xs font-semibold text-slate-400">
        ...
        {showAuthModal && (
          <div className="fixed inset-0 z-999 flex items-center justify-center p-4">
            ...
            <motion.div 
              ...
              className="relative w-full max-w-md bg-white/95 backdrop-blur-md rounded-2xl shadow-[0_30px_90px_rgba(15,23,42,0.2)] border border-slate-200 overflow-hidden flex flex-col p-6 text-center select-none"
            >
              {/* Close Button */}
              <button 
                onClick={() => setShowAuthModal(false)}
                className="absolute top-4 right-4 text-slate-400 hover:text-slate-700 transition-colors cursor-pointer bg-transparent border-0 outline-none"
              >
                <X className="w-5 h-5" />
              </button>

              <div className="mb-4">
                <div className="w-9 h-9 rounded-xl bg-blue-50 border border-blue-100 text-blue-600 flex items-center justify-center mx-auto mb-3 shadow-xs">
                ...
                {authTab === 'login' ? (
                  <span>
                    ...
                    <button 
                      ...
                      className="text-blue-600 hover:underline cursor-pointer bg-transparent border-0 p-0 outline-none font-bold"
                    >
                      Sign Up
                    </button>
                  </span>
